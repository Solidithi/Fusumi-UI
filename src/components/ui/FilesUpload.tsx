...
export function FileUpload({ accept = "*/*", multiple = false }: FileUploadProps) {
  ...
  const getTotalSize = () => {
    return files.filter((file) => selectedFiles.has(file.id)).reduce((total, file) => total + file.size, 0)
  }
              ...
              <div className="flex items-center space-x-3">
                ...
                {selectedFiles.size > 0 && (
                  <span className="text-white/60 text-xs">
                    {selectedFiles.size} selected ({formatFileSize(getTotalSize())})
                  </span>
                )}
              </div>
              ...
              {files.map((file) => (
                <motion.div
                  ...
                  className={`flex items-center justify-between p-2 rounded-lg backdrop-blur-sm min-h-[44px] transition-all duration-200 cursor-pointer ${
                    selectedFiles.has(file.id) ? "bg-white/20 border border-white/40" : "bg-white/10 hover:bg-white/15"
                  }`}
                  initial={{ opacity: 0, x: 20 }}
                  animate={{ opacity: 1, x: 0 }}
                  exit={{ opacity: 0, x: -20 }}
                ...
                >
                  <div className="flex items-center space-x-3 flex-1 min-w-0">
                    ...
                    >
                      {selectedFiles.has(file.id) && <Check className="h-2.5 w-2.5 text-gray-800" />}
                    </div>
